...
async function fetching(option) {
  const { key, image_type, orientation, safesearch, q, per_page, page } =
    option;
  const result = await axios(
    `?key=${key}&q=${q}&image_type=${image_type}&orientation=${orientation}&safesearch=${safesearch}&per_page=${per_page}&page=${page}`
  );
  return result.data;
}
...
async function searchAndDrawMarkUp(e) {
  ...
  if (searchInput === '') {
    return Notiflix.Notify.warning(`Search field empty!`);
  }
  try {
    ...
    error => {
      console.error(error);
      Notiflix.Notify.warning('Try again');
    };
  }
}
